...
│   │   └── passwordGenerator.ts # 密碼生成器核心邏輯
│   └── App.tsx        # 主應用組件
├── data/              # 數據存儲 (會自動創建)
│   ├── passwords.json # 密碼數據
│   └── master.json    # 主密碼數據
...
7. **密碼生成**: 
   - 點擊密碼輸入框旁的「生成」按鈕快速生成密碼
   - 點擊「⚙️」按鈕開啟高級密碼生成選項
   - 使用滑桿調整密碼長度 (4-64位)
   - 勾選需要的字符類型（大小寫字母、數字、符號）
   - 可選擇排除相似字符避免混淆
   - 自定義符號集合以符合特定網站要求
   - 查看實時密碼強度評估和熵值
   - 使用快速預設模板：
...
1. **字符集構建**: 根據用戶選擇的選項動態構建字符集
2. **必需字符保證**: 確保生成的密碼至少包含每種選定的字符類型
3. **隨機填充**: 使用 `Math.random()` 填充剩餘長度
4. **Fisher-Yates 洗牌**: 隨機打亂字符順序避免可預測的模式
...
// 排除相似字符版本
UPPERCASE_NO_SIMILAR: 'ABCDEFGHJKLMNPQRSTUVWXYZ' (排除 I, O)
LOWERCASE_NO_SIMILAR: 'abcdefghijkmnopqrstuvwxyz' (排除 l)
NUMBERS_NO_SIMILAR: '23456789' (排除 0, 1)
```
